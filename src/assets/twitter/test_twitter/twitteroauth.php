...
class TwitterOAuth {
  ...
  function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) {
    $this->sha1_method = new OAuthSignatureMethod_HMAC_SHA1_();
    $this->consumer = new OAuthConsumer_($consumer_key, $consumer_secret);
    if (!empty($oauth_token) && !empty($oauth_token_secret)) {
      $this->token = new OAuthConsumer_($oauth_token, $oauth_token_secret);
    } else {
      $this->token = NULL;
    }
  }
  ...
  function getRequestToken($oauth_callback) {
    ...
    $request = $this->oAuthRequest_($this->requestTokenURL(), 'GET', $parameters);
  ...
  function getAccessToken($oauth_verifier) {
    ...
    $request = $this->oAuthRequest_($this->accessTokenURL(), 'GET', $parameters);
    return $this->getToken($request);
  }
  ...
  function getXAuthToken($username, $password) {
    ...
    $request = $this->oAuthRequest_($this->accessTokenURL(), 'POST', $parameters);
    return $this->getToken($request);
  }
  ...
  function get($url, $parameters = array()) {
    $response = $this->oAuthRequest_($url, 'GET', $parameters);
  ...
  function post($url, $parameters = array()) {
    $response = $this->oAuthRequest_($url, 'POST', $parameters);
    if ($this->format === 'json' && $this->decode_json) {
      return json_decode($response);
    }
    return $response;
  ...
  function upload($url, $parameters = array()) {
    $response = $this->oAuthRequest_($url, 'POST', $parameters, true);
    if ($this->format === 'json' && $this->decode_json) {
      return json_decode($response);
    }
    return $response;
  ...
  function delete($url, $parameters = array()) {
    $response = $this->oAuthRequest_($url, 'DELETE', $parameters);
    if ($this->format === 'json' && $this->decode_json) {
      return json_decode($response);
    }
    return $response;
   ...
   */
  function oAuthRequest_($url, $method, $parameters, $upload = false) {
    ...
    if ($upload) {
      ...
      $request = OAuthRequest_::from_consumer_and_token($this->consumer, $this->token, $method, $url, $signable_parameters);
      $request->sign_request($this->sha1_method, $this->consumer, $this->token);
      $request->set_parameters($parameters);
    } else {
      $request = OAuthRequest_::from_consumer_and_token($this->consumer, $this->token, $method, $url, $parameters);
      $request->sign_request($this->sha1_method, $this->consumer, $this->token);
    }
  ...
  {
    $token = OAuthUtil_::parse_parameters($request);
	if(isset($token['oauth_token'], $token['oauth_token_secret']))
    	$this->token = new OAuthConsumer_($token['oauth_token'], $token['oauth_token_secret']);
	return $token;
  }
}
